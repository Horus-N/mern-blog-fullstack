...
export const createRefresh = async (user, dispatch, signInSuccess) => {
  ...
  try {
    ...
    if (decodedToken.exp < date.getTime() / 1000) {
      ...
      dispatch(signInSuccess({...res.user,token:res.token,refreshToken:res.refreshToken}));
    }
    return;
  } catch (error) {
 ...
 const refresh = async (token) => {
  try {
    ...
    const res = await request.post("http://localhost:5000/api/auth/refresh-token",{},config);
    return res.data
  } catch (error) {
    return error.response;
  }
